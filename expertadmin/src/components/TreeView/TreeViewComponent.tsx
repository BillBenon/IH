...
export default function TreeViewComponent(props: TreeViewComponentProps) {
  ...
  useEffect(() => {
    (async () => {
      const data = await BrowserStorage.getItem('expandedKeys');
      (data && data.length) ? setExpandedKeys(data) : setExpandedKeys(props.expandedKeys);
      const target = document.querySelector('.Pane');
      const currentTreeScrollPos = localStorage.getItem('treeScrollTop');
      if (currentTreeScrollPos && target) {
        target.scrollTop = +currentTreeScrollPos;
      }
    })()
  }, [props.expandedKeys])
  ...
  const onExpand = (data: any, node: any) => {
    ...
    BrowserStorage.setItem('expandedKeys', data);
